...
class MockingjayGenerator {
  ...
  _generateJsonObject(jsonObject, indent = 0) {
    if (Array.isArray(jsonObject)) {
      const children = jsonObject.map(obj => this._generateJsonObject(obj, indent + 1))
      return `[\n${'  '.repeat(indent)}${children.join(',\n')}]`
    }
    else if (typeof jsonObject === 'object') {
      const children = Object.keys(jsonObject).map(key => {
        return `${'  '.repeat(indent)}"${key}": ${this._generateJsonObject(jsonObject[key], indent + 1)}`
      })
      return `[\n${children.join(',\n')}]`
    }
